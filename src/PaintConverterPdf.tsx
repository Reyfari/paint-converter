...
import type { PaintData } from './converter'

type PaintResponse = PaintData

interface PaintEquivalentResult {
  paint: PaintResponse
  tier: 'direct' | 'derived'
      ...
      {equivs.map((g, i) => {
        ...
        return (
          <View key={`${g.brand}-${g.name}`} style={i > 0 ? s.cellAlt : undefined}>
            ...
            {allVariants.length > 0 && <Text style={s.variants}>{allVariants.join(', ')}</Text>}
  ...
  return (
    <Document>
      <Page size="A4" style={s.page}>
        ...
        <Text style={s.subtitle}>Generated {today} · Paint Converter</Text>

        <View style={s.table}>
          <View style={s.headerRow}>
            <View style={[s.headerCell, { width: sourceWidth }]}><Text>Source</Text></View>
            {data.vendorColumns.map(v => (
              <View key={v} style={[s.headerCell, { width: vendorWidth }]}><Text>{v}</Text></View>
            ))}
          </View>
          ...
          {data.rows.map(row => (
            <View key={row.source.id} style={s.row} wrap={false}>
              <View style={[s.sourceCell, { width: sourceWidth }]}>
                ...
                <View style={s.sourceCellInfo}><SourceBlock paint={row.source} /></View>
              </View>
              {data.vendorColumns.map(vendor => {
                const equivs = row.vendorCells.get(vendor)
                if (!equivs || equivs.length === 0) {
                  return <View key={vendor} style={[s.emptyCell, { width: vendorWidth }]}><Text style={s.emptyText}>—</Text></View>
                }
                return <View key={vendor} style={[s.equivCell, { width: vendorWidth }]}><EquivCellContent equivs={equivs} /></View>
              })}
            </View>
          ))}
        </View>
        ...
        <View style={s.disclaimer}>
          <Text style={s.disclaimerText}>
            <Text style={s.disclaimerBold}>Cross-brand equivalences</Text> are sourced from vendor-published conversion charts. Always compare in person before committing to a project.
          </Text>
          <Text style={s.disclaimerText}>
            <Text style={s.disclaimerBold}>Direct</Text> matches come from a single chart entry. <Text style={s.disclaimerBold}>Derived</Text> matches are inferred by chaining two entries and carry more uncertainty.
          </Text>
        </View>
      </Page>
    </Document>
